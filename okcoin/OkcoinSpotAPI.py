...
class OKCoinSpot:
    ...
    def kline(self, symbol = '', type = '5min', size = '100', since = '0000000000000'):
        ...
        params=''
        if symbol:
            params = 'symbol=%(symbol)s&type=%(type)s&size=%(size)s&since=%(since)s' %{'symbol':symbol,'type':type,'size':size,'since':since}
        return httpGet(self.__url,TRADES_RESOURCE,params)
